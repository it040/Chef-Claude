...
const PromptSidebar = ({ open, onClose }) => {
  ...
  const content = (
    <Box sx={{ width: drawerWidth, display: 'flex', flexDirection: 'column', height: '100%' }}>
              ...
              <List dense>
                {createdData.recipes.map((r) => (
                  <ListItemButton key={r._id} onClick={() => navigate(`/recipe/${r._id}`)}>
                    <ListItemText primaryTypographyProps={{ noWrap: true }} primary={r.title} secondary={new Date(r.createdAt).toLocaleDateString()} />
                    <IconButton size="small" onClick={(e) => { e.stopPropagation(); setRecipeMenu({ anchorEl: e.currentTarget, id: r._id }); }}>
      ...
      {/* Recipe menu */}
      <Menu anchorEl={recipeMenu.anchorEl} open={Boolean(recipeMenu.anchorEl)} onClose={() => setRecipeMenu({ anchorEl: null, id: null })} anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }} transformOrigin={{ vertical: 'top', horizontal: 'right' }}>
        <MenuItem onClick={() => { const id = recipeMenu.id; setRecipeMenu({ anchorEl: null, id: null }); navigate(`/recipe/${id}`); }}>
          <ListItemIcon><Send fontSize="small" /></ListItemIcon>
          Open
        </MenuItem>
        <MenuItem onClick={async () => { const id = recipeMenu.id; try { await navigator.clipboard.writeText(`${window.location.origin}/recipe/${id}`); } catch {} setRecipeMenu({ anchorEl: null, id: null }); }}>
          <ListItemIcon><ContentCopy fontSize="small" /></ListItemIcon>
          Copy Link
        </MenuItem>
        <MenuItem onClick={() => { const id = recipeMenu.id; setRecipeMenu({ anchorEl: null, id: null }); deleteRecipeMutation.mutate(id); }} disabled={deleteRecipeMutation.isPending}>
